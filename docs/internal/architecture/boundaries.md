...
## Hard Rules
1. Feature code must never import `@platform/ipc/*`.
2. Renderer code must never import `@/app/main/*`, `electron`, or `node:*`.
3. Desktop access in renderer must go through `@platform/desktop/client`.
4. `window.desktop` must only be used in `packages/shared/src/platform/desktop/client.ts`.
5. IPC keys stay centralized in `packages/shared/src/platform/ipc/keys.js` and are not duplicated inline.

## Desktop Contract Flow
1. `packages/shared/src/platform/desktop/types.ts` defines the canonical desktop API contract.
2. `apps/electron/src/preload/index.js` is the thin preload entrypoint and delegates desktop bridge exposure to `apps/electron/src/preload/desktop-bridge.js`.
3. `packages/shared/src/platform/desktop/client.ts` is the only renderer-side access point.
...
1. Add/update types in `packages/shared/src/platform/desktop/types.ts`.
2. Add preload implementation.
3. Add or update client methods in `packages/shared/src/platform/desktop/client.ts`.
4. Consume that method from feature/service code.

## PR Checklist
- [ ] No direct `window.desktop` usage outside `packages/shared/src/platform/desktop/client.ts`.
- [ ] No `@platform/ipc/*` imports from renderer/feature code.
- [ ] No renderer imports from `@/app/main/*`, `electron`, or `node:*`.
- [ ] IPC key strings only come from `packages/shared/src/platform/ipc/keys.js`.
