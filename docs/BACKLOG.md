...
_Trigger: next release that needs a number._
...
_Trigger: next substantial message-cache work, or as the template for the Solid build._
...
community membership. _Trigger: meaningful scale or measured bottleneck._
...
changes are infrequent. _Trigger: before 1.0, or when consolidating the remaining
postgres_changes subscriptions._
...
- **Fanout:** DB triggers on status change → `PRESENCE_CHANGE` to each friend's
  ...
  Trigger broadcasts the true status _except_ invisible → always `offline`.
...
- **Precondition:** rate-limit username changes to one per 24h _at the RPC level_
