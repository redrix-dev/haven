...
(the _why/what_ + dependency mapping); this doc is the _how/when_ + decision gates.
...
The **exit criteria** are the point — "how do we _know_ this is done and we're allowed
...
merges**, deleted on exit. The code is garbage by design — only the _findings_
...
**Goal:** answer the load-bearing "will it even work" questions _before_ committing
...
| 6   | **Voice**                 | Minimal LiveKit: join a room + hear audio in WKWebView. _Highest-risk item — a ❌ here halts the Tauri bet_                                                              | ✅     | **Full chain cleared.** Mic capture → Supabase sign-in → `voice-token` → LiveKit connect (WebRTC in WKWebView) → publish + subscribe → **two-way audio confirmed cross-device** (Tauri ↔ desktop).                                                                                                          |
...
**Goal:** turn the 6 probe results into a reliable go/no-go prediction for the _full_
build. This is the formal **Gate 1** — the decision on whether the stack is worth
committing to. We do this _immediately_ after Step 1; no continuation is planned
...
- **Probe 1** — a non-trivial _injected_ OS capability (spike proved the `ping` mechanism;
...
Supersedes the prior "shared-core hardening / Approach C" plan (preserved below under _Superseded_).

**The decision, in one line:** stop trying to share a _reactive cache_ across React and Solid.
...
- The Nexus pattern was lifted from the stoat/Revolt SDK, which runs it on **Solid's**
  fine-grained reactivity. That's _why_ the original has no snapshot caches / `revision`
...
- Haven ran the same pattern on **zustand** (coarse, manual reactivity) and **shared the
  ...
  shared reactive core grew illegible. The complexity is _substrate scar tissue_, not the
  pattern's fault.
- The rebuild targets **Solid** — the substrate the pattern was born for. So: share the
  _logic_ (pure functions, identical everywhere), let each platform own its _cache_ in its
...
> sharing of the _cache_ (not the logic) is exactly what The Cleave undoes. Its pure-logic
> extraction survives (§5 of the handoff); its dual-binding/shared-store half is retired.

- **Depth:** full extraction → shared core _zero-React_, per-domain CI-gated loop.
...
- **Service-class rename** `…Nexus` → `…ControllerNexus`, base `ControllerNexus.ts` extracted from
  repetition — _not pursued_; folded into the cleave's reasoning instead.
- **3c cleanups / 3d decomposition** (`communityDataBackend.ts` @ 2525 split) — still valid hygiene,
still their own gated steps, now framed as work on the _pure shared logic_ layer.
</details>
...
> layer)** — it must resolve cleanly from _both_ mobile and the Solid world. `solid-bindings` is being
...
- Metro + symlinked workspaces is _the_ historically painful part — it's literally why this repo
...
3. **Prerequisite/safety net:** the headless **`mobile:bundle`** gate (`expo export`, added with the
   ChannelNexus loop) is what makes this conversion _safe to attempt_ — it fails on any broken Metro/Babel
...
> bundled successfully — only when the **`tsconfig.json` `paths`** entry was _also_ removed did
...
> on a _total_ resolution failure: missing module, broken import, transform error, or an alias broken in
> **all** layers). Because the resolvers are redundant, it does **NOT** catch single-layer alias _drift_ —
...
The base swap is small _because_ the reactive methods are unused. The real per-platform
