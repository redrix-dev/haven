...
> for Tauri/Solid" = make it framework-neutral + add adapters) is the _old_ approach. Re-read each
> entry through the handoff's §0.4 test instead: **is it logic (→ stays in pure `shared`) or a cache
> (→ moves to a per-platform data layer)?** "🔧 decouple" entries are mostly _caches_ that should
...
"crap" is portable — it's a _future tidy_, not a Tauri blocker.
...
| `appHost.ts`                     | **differs** between the two | ⚠️ reconcile — _not_ a pure dup (which is canonical? mobile imports `@shared/platform/…`, others `@shared/infrastructure/platform/…`) |
