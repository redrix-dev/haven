...
Deno.serve(async (req) => {
  ...
  const { data, error } = await supabase
    .from("ota_releases")
    .select(
      "id, channel, runtime_version, bundle_url, manifest_url, created_at, is_active, ota_release_note, git_sha",
    )
    .eq("channel", channel)
  ...
  const manifestRoot = data.manifest_url
    ? getManifestRoot(data.manifest_url)
    : null;
    ...
    const manifest = {
      ...
      assets: await Promise.all(
        (platformMetadata.assets ?? []).map(async (asset) => {
          const extension =
            asset.ext?.trim().toLowerCase() || getFileExtension(asset.path);
      ...
      metadata: {},
      extra: {
        otaReleaseNote: data.ota_release_note,
        gitSha: data.git_sha,
      },
    };
