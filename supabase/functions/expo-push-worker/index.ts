...
const buildExpoMessage = (job: ExpoPushDispatchJobRow): ExpoPushMessage => {
  ...
  const priority: ExpoPushMessage["priority"] =
    job.kind === "dm_message" || job.kind === "channel_mention"
      ? "high"
      : "default";
...
const insertDeliveryTrace = async (
  ...
  if (error) {
    console.warn(
      "expo-push-worker failed to insert notification delivery trace:",
      {
        error: error.message,
...
const requireExpoAccessToken = (): string => {
  const token =
    Deno.env.get("HAVEN_EXPO_ACCESS_TOKEN")?.trim() ||
...
const isInvalidDeviceTicket = (ticket: ExpoTicket): boolean => {
  ...
  return (
    err.includes("devicenotregistered") || msg.includes("devicenotregistered")
  );
};
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return jsonResponse(
      {
        code: 500,
        message: error instanceof Error ? error.message : "Missing Expo token",
      },
      500,
    );
  }
  ...
  const maxJobs = clampInt(
    ...
    1,
    isCron ? 200 : 50,
    wakeSource === "wakeup" ? 20 : isCron ? 50 : 15,
  );
  ...
  const { data: claimRows, error: claimError } = await supabaseAdmin.rpc(
    "claim_expo_push_notification_jobs",
    {
      p_limit: maxJobs,
  ...
  if (claimError) {
    ...
    return jsonResponse(
      { code: 500, message: "Failed to claim expo push jobs" },
      500,
    );
  }
  ...
  if (dmJobsRequiringRecheck.length > 0) {
    ...
    if (recheckError) {
      console.warn(
        "expo-push-worker send-time recheck rpc failed; using claim snapshot:",
        recheckError,
      );
    } else {
    ...
    try {
      if (job.recipient_dismissed_at || job.recipient_read_at) {
        ...
        const { error: completeError } = await supabaseAdmin.rpc(
          "complete_expo_push_notification_job",
          {
            p_job_id: job.job_id,
            p_outcome: "skipped",
            p_error:
              "Notification already dismissed/read before expo push delivery.",
        ...
        if (completeError)
          console.error(
            "expo-push-worker failed to mark skipped job:",
            completeError,
          );
        stats.skipped += 1;
      ...
      if (!job.recipient_deliver_push) {
        const { error: completeError } = await supabaseAdmin.rpc(
          "complete_expo_push_notification_job",
          {
            ...
            p_outcome: "skipped",
            p_error:
              "Expo push delivery disabled by current preferences/flags.",
            p_retry_delay_seconds: 60,
            p_provider_status_code: null,
          },
        );
        if (completeError)
          console.error(
            "expo-push-worker failed to mark push-disabled job skipped:",
            completeError,
          );
        stats.skipped += 1;
        await insertDeliveryTrace(supabaseAdmin, {
      ...
      if (job.kind === "dm_message") {
        ...
        if (sendTimeRecheck && !sendTimeRecheck.shouldDeliverPush) {
          ...
          if (completeError)
            console.error(
              "expo-push-worker failed to mark send-time recheck skip:",
              completeError,
            );
          stats.skipped += 1;
          await insertDeliveryTrace(supabaseAdmin, {
  ...
  for (let i = 0; i < jobsToSend.length; i += BATCH) {
    ...
    try {
      ...
      for (const job of batch) {
        ...
        if (completeError)
          console.error(
            "expo-push-worker complete after HTTP error:",
            completeError,
          );
    ...
    for (let j = 0; j < batch.length; j++) {
      ...
      if (ticket.status === "ok") {
        ...
        if (completeError) {
          console.error(
            "expo-push-worker failed to complete sent job:",
            completeError,
          );
        } else {
          stats.sent += 1;
      ...
      if (terminalDevice) {
        ...
        if (delErr) {
          console.warn(
            "expo-push-worker failed to delete invalid expo subscription:",
            delErr.message,
          );
        } else {
      ...
      const { error: completeError } = await supabaseAdmin.rpc(
        "complete_expo_push_notification_job",
        {
          ...
          p_error: truncate(
            ticketMessage ||
              `Expo ticket status: ${ticket.status ?? "unknown"}`,
            3900,
          ),
      ...
      if (completeError) {
        console.error(
          "expo-push-worker failed to complete failed job:",
          completeError,
        );
      }
