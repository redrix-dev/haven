...
Deno.serve(async (req) => {
  ...
  const maxExpiredMessages = clampInt(
    body.maxExpiredMessages,
    1,
    isCron ? 500 : 100,
    isCron ? 200 : 50,
  );
  const maxDeletionJobs = clampInt(
    body.maxDeletionJobs,
    1,
    isCron ? 500 : 150,
    isCron ? 150 : 50,
  );
  ...
  if (cleanupError) {
    console.error(
      "message-media-maintenance cleanup rpc failed:",
      cleanupError,
    );
    return jsonResponse(
      { code: 500, message: "Failed to cleanup expired message attachments" },
      500,
    );
  }
  deletedChannelMessages = Number(cleanupCount ?? 0) || 0;

  const { data: cleanupDmCount, error: cleanupDmError } =
    await supabaseAdmin.rpc("cleanup_expired_dm_message_attachments", {
      p_limit: maxExpiredMessages,
    });
  if (cleanupDmError) {
    console.error(
      "message-media-maintenance DM cleanup rpc failed:",
      cleanupDmError,
    );
    return jsonResponse(
      { code: 500, message: "Failed to cleanup expired DM image attachments" },
      500,
    );
  }
  ...
  if (claimError) {
    ...
    return jsonResponse(
      { code: 500, message: "Failed to claim media deletion jobs" },
      500,
    );
  }
    ...
    try {
      ...
      if (removeError) {
        if (isMissingObjectError(removeError.message ?? "")) {
          ...
          if (completeErr)
            console.error(
              "Failed completing missing-object deletion job as done",
              completeErr,
            );
          deletedObjects += 1;
          continue;
      ...
      const retryDelaySeconds = shouldDeadLetter
        ? 300
        : Math.min(900, 30 * Math.max(job.attempts ?? 1, 1));
      ...
      if (completeErr) {
        console.error(
          "Failed updating deletion job failure outcome:",
          completeErr,
        );
      }
