...
Deno.serve(async (req) => {
  ...
  if (!isUuid(communityId) || !isUuid(channelId)) {
    return jsonResponse(
      { code: 400, message: "communityId and channelId must be UUIDs" },
      400,
    );
  }
  ...
  if (channelError) {
    ...
    return jsonResponse(
      { code: 500, message: "Failed to validate channel access" },
      500,
    );
  }
  if (!channelRow) {
    return jsonResponse(
      { code: 403, message: "Not authorized for this channel" },
      403,
    );
  }
  if (channelRow.kind !== "text") {
    return jsonResponse(
      { code: 400, message: "Backfill only applies to text channels" },
      400,
    );
  }
  ...
  const requestedIds = Array.isArray(body.messageIds)
    ? Array.from(
        new Set(
          body.messageIds.filter((value): value is string => isUuid(value)),
        ),
      )
    : [];
  ...
  if (messagesError) {
    console.error(
      "link-preview-backfill messages query failed:",
      messagesError,
    );
    return jsonResponse(
      { code: 500, message: "Failed to load messages for backfill" },
      500,
    );
  }
  ...
  if (previewsError) {
    console.error(
      "link-preview-backfill previews query failed:",
      previewsError,
    );
    return jsonResponse(
      { code: 500, message: "Failed to load preview rows for backfill" },
      500,
    );
  }
  ...
  const alreadyPresent = messageIds.filter(
    (id) => previewByMessageId.get(id)?.status === "ready",
  ).length;
  const toQueue = messageIds.filter(
    (id) => previewByMessageId.get(id)?.status !== "ready",
  );
  ...
  const { data: enqueueRows, error: enqueueError } = await admin.rpc(
  ...
  if (enqueueError) {
    ...
    return jsonResponse(
      { code: 500, message: "Failed to enqueue link preview jobs" },
      500,
    );
  }
  ...
  const queued = Array.isArray(enqueueRows)
    ? enqueueRows.filter(
        (row) =>
          ...
          typeof row === "object" &&
          "queued" in row &&
          (row as { queued: unknown }).queued === true,
      ).length
    : 0;
