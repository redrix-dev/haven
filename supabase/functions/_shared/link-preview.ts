...
const fetchWithTimeout = async (input: string, init?: RequestInit) => {
  ...
  try {
    return await fetch(input, {
      ...
      headers: {
        "User-Agent":
          "HavenLinkPreviewBot/1.0 (+https://github.com/redrix-dev/haven)",
        ...(init?.headers ?? {}),
      },
    });
  } catch (error) {
    ...
    throw new RetryablePreviewError(
      `Preview fetch failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  } finally {
...
const buildGenericHtmlPreview = async (
  rawUrl: string,
  ...
  const canonicalUrl =
    toAbsoluteUrl(meta["og:url"] ?? null, finalUrl) ?? finalUrl;
  const thumbnailSourceUrl = toAbsoluteUrl(
  ...
  const hasMeaningfulContent = Boolean(
    snapshot.title || snapshot.description || snapshot.thumbnail,
  );
...
const buildYoutubePreview = async (
  rawUrl: string,
  url: URL,
  videoId: string,
): Promise<ResolvedPreview> => {
  ...
  const siteName =
    typeof data.provider_name === "string" ? data.provider_name : "YouTube";
...
const buildVimeoPreview = async (
  ...
  const siteName =
    typeof data.provider_name === "string" ? data.provider_name : "Vimeo";
  const thumbnailSourceUrl =
...
export const resolvePreviewForUrl = async (
  rawUrl: string,
): Promise<ResolvedPreview> => {
  const safeUrl = assertSafeFetchUrl(rawUrl);
...
export const mirrorPreviewThumbnail = async (params: {
  ...
  const namespace = (await sha256HexFromString(params.normalizedUrl)).slice(
    0,
    8,
  );
  const ext = pickImageExtension(
    contentTypeHeader || null,
    params.thumbnailSourceUrl,
  );
  const objectPath = `${namespace}/${hash}.${ext}`;

  const { error } = await params.supabaseAdmin.storage
  ...
  if (error) {
    throw new RetryablePreviewError(
      `Failed to mirror thumbnail: ${error.message}`,
    );
  }
