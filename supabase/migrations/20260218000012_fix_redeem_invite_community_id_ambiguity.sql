-- Fix ambiguous `community_id` reference inside invite redeem RPC.
-- Root cause: function output column name `community_id` can conflict with SQL identifiers
-- in PL/pgSQL statements (notably `on conflict (community_id, user_id)`).
--
-- Strategy:
-- 1) Prefer column resolution when variable/column names collide.
-- 2) Use ON CONFLICT ON CONSTRAINT for deterministic conflict target.
