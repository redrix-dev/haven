...
const buildMarkdownReport = (input: {
  ...
  const statusIcon = getStateIcon(
    hasFailures ? "failed" : reason === "interrupted" ? "pending" : "passed",
  );
  ...
  for (const testModule of testModules) {
    ...
    lines.push(
      `### ${getStateIcon(fileState)} \`${testModule.relativeModuleId}\``,
    );
    lines.push("");
    lines.push(`- **State:** ${fileState}`);
    lines.push(
      `- **Duration:** ${formatDuration(testModule.diagnostic().duration)}`,
    );
    lines.push(
    ...
    if (tests.length > 0) {
      ...
      for (const testCase of tests) {
        ...
        const durationLabel =
          duration !== undefined ? ` _${formatDuration(duration)}_` : "";
        const skipNote =
        ...
        if (result.state === "failed" && result.errors.length > 0) {
          for (const error of result.errors) {
            ...
            lines.push("  ```text");
            lines.push(
              `  ${sanitizeFence(formatUnknownError(error)).replace(/\r?\n/g, "\n  ")}`,
            );
            lines.push("  ```");
          }
        }
...
export default class VitestMarkdownReporter implements Reporter {
  ...
  async onTestRunEnd(
    ...
    const shouldAppend =
      append && fs.existsSync(outputFile) && fs.statSync(outputFile).size > 0;
