...
const mobileThemesPath = path.join(
  rootDir,
  "apps/mobile/uniwind-themes.generated.cjs",
);
const mobileUniwindTypesPath = path.join(
  rootDir,
...
function generateMobileCssBridgeBlock(): string {
  ...
  const lines: string[] = [MOBILE_START_MARKER, "@layer theme {", "  :root {"];
  ...
  for (const builtinThemeId of ["light", "dark"]) {
    ...
    for (const variableName of variableNames) {
      ...
      if (typeof value !== "string" || value.length === 0) {
        throw new Error(
          `Theme "${builtinThemeId}" is missing "${variableName}".`,
        );
      }
      lines.push(`      --${variableName}: ${value};`);
    }
...
function resolveReusablesCssVariables(
  themeId: BuiltinThemeId,
): Record<string, string> {
  const variables = resolveThemeVariables(themeId);
  ...
  for (const name of REUSABLES_CSS_VARIABLES) {
    ...
    if (typeof value !== "string" || value.length === 0) {
      throw new Error(
        `Theme "${themeId}" is missing reusables variable "${name}".`,
      );
    }
    resolved[name] = value;
  }
...
function buildReusablesThemePalette(
  ...
  const pick = (semantic: string, cssName?: string) => {
    const value =
      variables[semantic] ?? (cssName ? variables[cssName] : undefined);
    if (typeof value !== "string" || value.length === 0) {
...
function generateReusablesThemeModule(): string {
  const lightPalette = buildReusablesThemePalette(
    resolveReusablesCssVariables("default"),
  );
  const darkPalette = buildReusablesThemePalette(
    resolveReusablesCssVariables("default"),
  );
...
function main(): void {
  ...
  const nextGlobals = replaceBetweenMarkers(
    globalsSource,
    generateCssBridgeBlock(),
  );
