...
import { VoiceBrowserPortalPopout } from "./VoiceBrowserPortalPopout";
...
export function VoiceDock() {
  ...
  createEffect(() => {
    if (!voice.activeChannel || (!voice.joined && !voice.joining)) return;
    void bridge.preparePopout?.("/popout/voice", popoutOptions);
  });

  const openRoutePopout = () => {
    void bridge.openPopout("/popout/voice", popoutOptions);
  };

  const openPopout = () => {
    if (bridge.capabilities?.browserPortalPopout) {
      setPortalOpen(true);
      return;
    }
    openRoutePopout();
  ...
  return (
    <Show when={voice.activeChannel}>
      {(channel) => (
        <>
          <VoiceBrowserPortalPopout
            open={portalOpen()}
            onOpenChange={setPortalOpen}
            onFallback={openRoutePopout}
          />
