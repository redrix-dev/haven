...
type ChannelNameRow = Pick<
  Database["public"]["Tables"]["channels"]["Row"],
  "id" | "name"
>;

type RoleNameRow = Pick<
  Database["public"]["Tables"]["roles"]["Row"],
  "id" | "name"
>;

type MemberRoleRow = Pick<
  Database["public"]["Tables"]["member_roles"]["Row"],
  "role_id"
>;

const SERVER_VISIBLE_DESTINATIONS: SupportReportDestination[] = [
  "server_admins",
];
...
const parseSupportReportSnapshot = (
  value: unknown,
): SupportReportSnapshot | null => {
...
const parseInternalNotes = (
  ...
  return rawNotes
    .map((entry) => {
      ...
      const authorUserId =
        typeof note.authorUserId === "string" ? note.authorUserId : null;
      const body = typeof note.body === "string" ? note.body : null;
      const createdAt =
        typeof note.createdAt === "string" ? note.createdAt : null;
...
const mapSupportReportRowToSummary = (
...
const loadChannelNameMap = async (
  sb: HavenSupabaseClient,
  ...
  return new Map(
    (data ?? []).map((row) => [row.id, (row as ChannelNameRow).name]),
  );
};

const loadEscalationContext = async (
  sb: HavenSupabaseClient,
  communityId: string,
  userId: string,
) => {
  ...
  if (membership?.id) {
    ...
    const roleIds = (memberRoles ?? []).map(
      (row) => (row as MemberRoleRow).role_id,
    );
    if (roleIds.length > 0) {
      ...
      roleNames = (roleRows ?? [])
        .map((row) => (row as RoleNameRow).name)
        .sort();
    }
  }
...
const fetchSupportReportRow = async (
  sb: HavenSupabaseClient,
  reportId: string,
) => {
  const { data, error } = await sb
...
export function createServerModmailBackend(
  ...
  const modmail: ServerModmailBackend = {
    async listServerReports(communityIds) {
      ...
      return ((data ?? []) as SupportReportRow[]).map(
        mapSupportReportRowToSummary,
      );
    },
    ...
    async getServerReport(reportId) {
      ...
      const messageIds = (
        (messageLinks ?? []) as SupportReportMessageLinkRow[]
      ...
      if (messageIds.length > 0) {
        ...
        linkedMessages = messageIds.map((messageId) => {
          const messageRow =
            messageRows.find((row) => row.id === messageId) ?? null;
          return {
            messageId,
      ...
      return {
        ...
        targetDisplayName: getTargetDisplayName(
          summary.reportType,
          notes,
          summary.snapshot,
        ),
    ...
    async updateReportStatus(reportId, status) {
      ...
      if (!reportRow.community_id) {
        throw new Error(
          "Platform-only reports do not have a community status broadcast.",
        );
      }
      ...
      try {
        ...
        console.error(
          "Failed to broadcast report status update:",
          broadcastError,
        );
      }
    },
    ...
    async escalateReport(reportId) {
      ...
      if (!reportRow.community_id) {
        throw new Error(
          "Platform-only reports cannot be escalated through server modmail.",
        );
      }
      if (reportRow.destination !== "server_admins") {
        throw new Error(
          "Only server-staff-only reports can be shared with Haven staff.",
        );
      }

      const parsedOriginalNotes = parseJsonRecord(reportRow.notes);
      const [
        { data: originalChannelLinks, error: channelLinksError },
        { data: originalMessageLinks, error: messageLinksError },
        { data: originalReporter, error: originalReporterError },
      ] = await Promise.all([
        client
          .from("support_report_channels")
          .select("channel_id")
      ...
      const escalationContext = await loadEscalationContext(
        client,
        communityId,
        user.id,
      );
      ...
      const nextChannelLinks = (
        (originalChannelLinks ?? []) as Array<{ channel_id: string }>
      ).map((row) => ({
      ...
      const nextMessageLinks = (
        (originalMessageLinks ?? []) as Array<{ message_id: string }>
      ).map((row) => ({
        report_id: escalatedReportId,
