...
export function resolveTheme(
  themeRegistry: HavenThemeRegistry,
  ...
  const requested = options.selectedThemeId
    ? themeRegistry[options.selectedThemeId]
    : undefined;

  if (
    requested &&
    requested.status !== "disabled" &&
    (!requested.entitlementKey ||
      allowedEntitlements.has(requested.entitlementKey))
  ) {
    return requested;
  }
