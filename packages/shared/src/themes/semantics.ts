...
export function resolveSemanticEntries(tokens: HavenThemeTokens): Record<string, string> {
  const proxy = createThemeProxy(tokens);
  const entries: Record<string, string> = {};
  for (const semantic of Object.keys(semanticToPrimitive) as SemanticTokenName[]) {
