import type { NotificationItem, NotificationKind } from '@shared/lib/backend/types';
 ...
 */
export const NOTIFICATION_INBOX_EXCLUDED_KINDS: ReadonlySet<NotificationKind> = new Set([
  'dm_message',
  'friend_request_received',
  'friend_request_accepted',
]);

export function isNotificationInboxRow(notification: NotificationItem): boolean {
  if (notification.dismissedAt) return false;
  return !NOTIFICATION_INBOX_EXCLUDED_KINDS.has(notification.kind);
}

export function filterNotificationsForInbox(notifications: NotificationItem[]): NotificationItem[] {
  return notifications.filter(isNotificationInboxRow);
}

/** Unread = no readAt (matches desktop row semantics). */
export function countFilteredUnreadInInbox(notifications: NotificationItem[]): number {
  let n = 0;
