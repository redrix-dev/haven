...
describe("banVisibility", () => {
  it("filters channel access revocation content only in the targeted channel", () => {
    ...
    const placeholderReply = filtered.messages.find(
      (message) => message.id === "revoked-reply",
    );
    ...
    expect(
      isModerationRemovedReplyPlaceholder(placeholderReply as Message),
    ).toBe(true);
    expect(filtered.reactions.map((reaction) => reaction.id)).toEqual([
      "reaction-2",
    ]);
    expect(filtered.attachments.map((attachment) => attachment.id)).toEqual([
      "attachment-2",
    ]);
    expect(filtered.linkPreviews.map((preview) => preview.id)).toEqual([
      "preview-2",
    ]);
  });
  ...
  it("hides blocked user content without placeholders and removes descendant replies", () => {
    const visibleRoot = makeMessage("visible-root", "user-1", "Visible root");
    const blockedThreadRoot = makeMessage(
      "blocked-root",
      "blocked-user",
      "Blocked root",
    );
    const blockedThreadReply = makeMessage(
      "blocked-thread-reply",
      "user-2",
      "Reply in blocked thread",
      {
        replyToMessageId: "blocked-root",
      },
    );
    const blockedReply = makeMessage(
      "blocked-reply",
      "blocked-user",
      "Blocked reply",
      {
    ...
    const visibleReply = makeMessage(
      "visible-reply",
      "user-3",
      "Visible reply",
      {
    ...
    const filtered = filterBlockedUserContent(
      {
        ...
        linkPreviews: [
          makeLinkPreview("preview-1", "blocked-reply"),
          makeLinkPreview("preview-2", "visible-reply"),
        ],
      },
    ...
    expect(
      filtered.messages.some(
        (message) => message.content === BANNED_REPLY_PLACEHOLDER_CONTENT,
      ),
    ).toBe(false);
    expect(filtered.reactions.map((reaction) => reaction.id)).toEqual([
      "reaction-3",
  ...
  it("leaves blocked user content visible for elevated viewers", () => {
    const blockedMessage = makeMessage(
      "blocked-root",
      "blocked-user",
      "Blocked root",
    );
    ...
    const filtered = filterBlockedUserContent(
      {
        ...
        attachments: [
          makeAttachment("attachment-1", "blocked-root", "blocked-user"),
        ],
