...
function getBrowserViewportState(previousLayoutViewportHeightPx: number): MobileViewportState {
  ...
  if (!visualViewport || typeof window === 'undefined') {
    ...
    return {
      ...
      keyboardInsetPx: 0,
      keyboardOpen: false,
      layoutViewportHeightPx: fallbackHeight,
      scale: 1,
      shellHeightPx: fallbackHeight,
      visualViewportHeightPx: fallbackHeight,
      visualViewportOffsetTopPx: 0,
    };
  }
  ...
  const nextLayoutViewportHeightPx = hasFocusedTextEntry
    ? Math.max(previousLayoutViewportHeightPx, measuredLayoutViewportHeightPx)
...
export function MobileViewportProvider({ children }: { children: React.ReactNode }) {
  ...
  const [viewportState, setViewportState] = React.useState<MobileViewportState>(
    () => runtime.keyboard?.getState() ?? getInitialViewportState()
  );
  ...
  React.useEffect(() => {
    ...
    const syncBrowserViewportState = () => {
      frameId = null;
      commitState(getBrowserViewportState(layoutViewportHeightPx));
    };
    ...
    const scheduleSync = () => {
      ...
      frameId = window.requestAnimationFrame(syncBrowserViewportState);
    };

    if (runtime.keyboard && runtime.capabilities.nativeKeyboard) {
      const unsubscribe = runtime.keyboard.subscribe((nextState) => {
