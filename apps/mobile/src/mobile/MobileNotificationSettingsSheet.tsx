...
export function MobileNotificationSettingsSheet({
  ...
  const usingNativePush = runtime.notifications.transport === 'native';
  ...
  const pushLabel = (() => {
    ...
    if (pushBlocked) return usingNativePush ? 'Blocked by device settings' : 'Blocked by browser';
  ...
  const pushRowDescription = (() => {
    if (pushBlocked) {
      return usingNativePush
        ? 'Enable notifications for Haven in your device settings first.'
        : 'Enable notifications for this site in browser settings first.';
    }
    if (webPushStatus && !webPushStatus.supported) {
      return usingNativePush
        ? 'Native push is not available in this build.'
        : 'This browser or install mode does not support web push.';
    }
              ...
              {pushBlocked ? (
                <div className="rounded-xl bg-red-500/10 border border-red-500/20 px-4 py-3">
                  ...
                  <p className="text-xs text-red-400/70 mt-0.5">
                    {usingNativePush
                      ? 'Open your device settings to allow notifications for Haven.'
