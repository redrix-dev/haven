...
describe("DirectMessageNexus", () => {
  ...
  it("can load conversations without toggling the loading state", async () => {
    let resolveList: (
      value: DirectMessageConversationSummary[],
    ) => void = () => {};
  ...
  it("upsertMessage adds a new entry to its conversation", () => {
    const nexus = new DirectMessageNexus(
  ...
  it("upsertMessage keeps the conversation sorted oldest-first", () => {
    ...
    nexus.upsertMessage(
      message({ messageId: "m3", createdAt: "2026-01-01T00:00:03.000Z" }),
    );
    nexus.upsertMessage(
      message({ messageId: "m2", createdAt: "2026-01-01T00:00:02.000Z" }),
    );
  ...
  it("receiveMessage hydrates the exact realtime message by id", async () => {
    ...
    expect(nexus.reactiveStore.getState().messageEntities["m42"]?.content).toBe(
      "hydrated",
    );
  });
