...
This classifies **runtime importability from React Native** (Metro), not “could we rewrite it.” Anything under `app/ui/` or feature `components/` is treated as **web UI** unless noted.
...
| Most `lib/backend/*.ts` **logic**                                                                                                                                                                        | RPC/data shapes are fine **if** `@shared/lib/supabase` is replaced or adapted (see below).                                                                         |
...
| ------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `@shared/lib/supabase`                                                                      | Reads `process.env.SUPABASE_URL` / `SUPABASE_ANON_KEY` at **module load** (throws if missing). Webpack/Vite inject these; Expo expects `EXPO_PUBLIC_*` or `extra` + a thin native `createClient`. **Adapter:** `createSupabaseMobile.ts` in `apps/mobile` + optional shared `createSupabaseClient({ url, key })` refactor. |
...
| `@shared/stores/permissionsStore.ts`                                                        | Imports `getCommunityDataBackend` → `supabase`. **Adapter:** same as supabase; store is otherwise fine.                                                                                                                                                                                                                    |
...
| Area                                                  | Examples                                                                                                                                                                                 |
| ----------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Entire `app/ui/*`                                     | Radix / shadcn primitives, `cmdk`, `lucide-react`, DOM.                                                                                                                                  |
...
- `@shared/lib/createHavenSupabaseClient` — used by `apps/mobile/src/supabase/getMobileSupabase.ts` with AsyncStorage; web still uses `@shared/lib/supabase` singleton built from the same factory.
